...
export default function ImportarQuestoes() {
  ...
  const handleImportar = async () => {
    ...
    Alert.alert("Confirmar importação", `Importar ${preview.length} questões para o banco da OSI?`, [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            if (error) throw new Error(error.message);
            if (data?.error) throw new Error(data.error);
            Alert.alert("Sucesso!", `${data.importadas} questões importadas.`, [{ text: "OK", onPress: () => router.back() }]);
          } catch (err: any) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg }]} contentContainerStyle={{ paddingBottom: 60 }}>
      <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>Importar via Planilha</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <View style={[styles.infoCard, { backgroundColor: "#EFF6FF", borderColor: "#BFDBFE" }]}>
        <View style={styles.infoHeader}>
          ...
          <Text style={[styles.infoTitle, { color: colors.primary }]}>Como usar</Text>
        </View>
        ...
        <View style={styles.colunasList}>
          {[
            ["A", "enunciado", true], ["B", "opcao_a", true], ["C", "opcao_b", true],
            ["D", "opcao_c", false], ["E", "opcao_d", false], ["F", "opcao_e", false],
            ["G", "resposta_correta (A-E)", true], ["H", "justificativa", true],
            ["I", "materia", false], ["J", "dificuldade (Fácil/Média/Difícil)", false], ["K", "referencias", false],
          ].map(([col, nome, obrig]) => (
            <View key={String(col)} style={styles.colunaRow}>
              <Text style={[styles.colunaLetra, { backgroundColor: colors.primary }]}>{col}</Text>
      ...
      <TouchableOpacity
        style={[styles.btnPreview, { borderColor: colors.primary, backgroundColor: colors.card }, loading && { opacity: 0.7 }]}
      ...
      >
        {loading ? (
          ...
          <>
            ...
            <Text style={[styles.btnPreviewText, { color: colors.primary }]}>Pré-visualizar questões</Text>
          </>
        )}
      </TouchableOpacity>
      ...
      {preview !== null && (
        <View style={styles.previewSection}>
          <View style={styles.previewHeader}>
            <Text style={[styles.previewTitle, { color: colors.text }]}>{preview.length} questões encontradas</Text>
            <TouchableOpacity
              style={[styles.btnImportar, { backgroundColor: colors.primary }, importando && { opacity: 0.7 }]}
          ...
          {preview.map((q, i) => (
            <View key={i} style={[styles.questaoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <View style={styles.questaoCardHeader}>
                <Text style={[styles.questaoNum, { color: colors.textLight }]}>#{i + 1}</Text>
                ...
                ]}>{q.dificuldade || "Média"}</Text>
              </View>
              <Text style={[styles.questaoEnunciado, { color: colors.text }]} numberOfLines={3}>{q.enunciado}</Text>
              <View style={styles.opcoesRow}>
                {["a", "b", "c", "d", "e"].map((l) => {
                  if (!q[`opcao_${l}`]) return null;
                  const isCorreta = q.resposta_correta === l.toUpperCase();
                  return (
                    <View key={l} style={[styles.opcaoChip, isCorreta && { backgroundColor: "#10B981" }]}>
                      <Text style={[styles.opcaoChipText, isCorreta && { color: "#fff" }]}>{l.toUpperCase()}</Text>
                    </View>
                  );
                })}
              </View>
...
const styles = StyleSheet.create({
  ...
  colunaLetra: { width: 24, height: 24, borderRadius: 6, color: "#fff", fontSize: 12, fontWeight: "bold", textAlign: "center", lineHeight: 24 },
  colunaLabel: { flex: 1, fontSize: 12, color: "#1E3A5F" },
  obrig: { fontSize: 10, color: "#3B82F6", fontWeight: "600" },
  label: { fontSize: 14, fontWeight: "bold", marginHorizontal: 20, marginBottom: 8 },
  input: { borderWidth: 1, borderRadius: 12, padding: 14, marginHorizontal: 20, fontSize: 13, minHeight: 60, textAlignVertical: "top" },
  btnPreview: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, margin: 20, padding: 16, borderRadius: 14, borderWidth: 2 },
  ...
  btnImportar: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12 },
  btnImportarText: { color: "#fff", fontWeight: "bold", fontSize: 13 },
  questaoCard: { borderRadius: 14, padding: 14, marginBottom: 10, elevation: 1, borderWidth: 1 },
  questaoCardHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 },
  questaoNum: { fontSize: 12, fontWeight: "bold" },
  questaoTag: { fontSize: 10, fontWeight: "600", backgroundColor: "#F1F5F9", color: "#64748B", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  questaoEnunciado: { fontSize: 13, lineHeight: 19, marginBottom: 10 },
  opcoesRow: { flexDirection: "row", gap: 6 },
  opcaoChip: { width: 28, height: 28, borderRadius: 8, backgroundColor: "#F1F5F9", justifyContent: "center", alignItems: "center" },
